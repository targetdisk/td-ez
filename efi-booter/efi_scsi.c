...
bool efi_scsi_data_in (uint8_t lun, __xdata uint8_t *data, uint16_t length) __reentrant
{
  ...
  if(current_op == SCSI_OPERATION_REQUEST_SENSE) {
    ...
    /* We're a removable device. */
    data->rmb = true;

    /* Our identification. */
    xmemcpy( data->t10_vendor_identification, (__xdata void *)"Andrea R",          8 );
    xmemcpy( data->product_identification,    (__xdata void *)"targetdisk (SPI)",  16 );
  ...
  else if ( current_op == SCSI_OPERATION_READ_CAPACITY )
  {
    ...
    data->returned_logical_block_address  = bswap32(TOTAL_SECTORS - 1);
  ...
  else if ( current_op == SCSI_OPERATION_READ_10 )
  {
    ...
    block_addr = block_index * BYTES_PER_SECTOR;
    spiflash_read( block_index, buffer, BYTES_PER_SECTOR );
